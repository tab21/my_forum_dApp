...
export default function Home() {
	const postManagerContract = "0x8360c40e1695961f4d2C7475829C8713e2Cbd148"; //postManager smart contract address
	...
	async function getAllPosts() {
		...
		if (ethereum) {
			...
			const allPostsAddresses =
				await postManagerContractInstance.getPosts();
			...
			for (let i = 0; i < allPosts.posterAddress.length; i++) {
				...
				let noOfComments: number =
					allPosts.numberOfComments[i].toNumber();
				let postSCAddress = allPostsAddresses[i];

				//get postId
				const postid = await postManagerContractInstance.postIDs(
					postSCAddress
				);
			...
			<div className={styles.dropZoneStyle} {...getRootProps()}>
				...
				{isDragActive ? (
					...
					<p>
						...
						file ((Only *.jpeg, *jpg and *.png images will be
						accepted))
					</p>
				)}
			</div>
		...
		return (
			<div className={styles.activePostContainer}>
				<div>
					...
					/>
					<h4 className={styles.activePostText}>
						...
						<h4 className={styles.commentHeading}>
							{(() => {
								if (postData.comments.length === 1) {
									return (
										<div>{`${postData.comments.length} Comment`}</div>
									);
								} else {
									return (
			...
			>
				...
				<div>
					{activePost != null ? (
						...
						<>
							<div>
								{allPosts.map((post) => renderAllPosts(post))}
							</div>
							<div className={styles.createPostContainer}>
								<h2 className={styles.createPostText}>
									Create New Post{" "}
								</h2>
								<MyDropzone />
								<p
